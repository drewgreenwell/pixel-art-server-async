...
## Server
 ...
 Clients make only 2 calls to the server:
  -  On boot, clients attempt a connection to `/checkin`, passing the `id` stored in their firmware. This registers a new client or updates an existing client's IP Address. This process allows the client to be configured in the administration interface. *Note:* Clients can also be added directly in the admin interface for server only streaming mode. 
