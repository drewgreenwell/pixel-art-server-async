            ...
            for (let rowIndex = 0; rowIndex < client.height; rowIndex++) {
                ...
                for (let col = 0; col < client.width; col++) {
                    ...
                    if (quantizePalette && palettes.colorMap) {
                        rgb = palettes.colorMap.map(rgb);
                    }
                    if (useHexPalette) {
                        let hex = getHex(rgb); //.substring(2);
                        let idx = outPalette.indexOf(hex);
                        if (idx == -1) {
                            idx = 0;
                        }
                        pixels.push(idx);
                ...
                const row = { row: rowIndex, frame: frameIndex, duration, pixels };
                // const row = pixels;
                memo.push(row);
            }
            return memo;
...
function getPalettes(pixels, limit) {
    let result = { hex: [], rgb: [] };
    ...
    result.colorMap = quantize(pixels, limit);
    return result;
}
function getHex(p) {
...
function collapsePixels(pixelRows) {
...
//# sourceMappingURL=get_image_data.js.map