...
import {
  ...
  addAnImageStat,
  returnAnImageStat,
  returnAnImageStatFromBuffer,
} from '../utils/get_image_stat.js';
...
export async function editFile(req: Request, res: Response) {
  ...
  if (!uploadedFile.name.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
    return res.status(400).send({ success: false, error: 'Must have an image extension!' });
  }
  if (!uploadedFile.mimetype.startsWith('image/')) {
    return res.status(400).send({ success: false, error: 'Must be an image!' });
  }
  ...
  getImageData(Data.wledConfig.client, 256, /* useHexPalette */ true, null, fileBuffer).then(
    (result) => {
      if (result.data) result.data.meta.path = newName;
      res.send({
        success: true,
