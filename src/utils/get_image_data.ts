...
export async function getImageData(
  ...
  paletteLength: number = 512,
  useHexPalette: boolean = true,
  ...
  return pixelBufferOp.then((result) => {
    ...
    const rows = _.reduce(
      frames,
      (memo: ImageRow[], frameData, frameIndex) => {
        // default to 15 FPS
    ...
    console.log(
      `>> server returns ${path} with ${rows.length} rows over ${frames.length} frames and ${outPalette.length} palette colors`,
    );
    ...
    let durations =
      Array.isArray(metadata?.delay) && metadata.delay.length === frames.length
        ? metadata.delay
        : frames.length == 1
          ? [0]
